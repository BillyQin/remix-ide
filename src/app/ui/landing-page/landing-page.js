...
export class LandingPage extends BaseApi {
  ...
  render () {
    let load = function (item, examples) {
      ...
      modalDialogCustom.prompt(null, msg, null, (target) => {
        if (target !== '') {
        ...
        <div id="col1" class="col-sm-6">
          ...
          <div class="file">
            ...
            <div class="btn-group">
              ...
              <button class="btn btn-sm btn-secondary" onclick=${() => { load('Github URL', ['https://github.com/OpenZeppelin/openzeppelin-solidity/blob/master/contracts/crowdsale/Crowdsale.sol', 'https://github.com/OpenZeppelin/openzeppelin-solidity/blob/67bca857eedf99bf44a4b6a0fc5b5ed553135316/contracts/access/Roles.sol', 'github:OpenZeppelin/openzeppelin-solidity/contracts/ownership/Ownable.sol#v2.1.2']) }}>Github</button>
              <button class="btn btn-sm btn-secondary" onclick=${() => { load('bzz-raw URL', ['bzz-raw://<swarm-hash>']) }}>Swarm</button>
              <button class="btn btn-sm btn-secondary" onclick=${() => { load('ipfs URL', ['ipfs://<ipfs-hash>']) }}>Ipfs</button>
